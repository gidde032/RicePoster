...
def test_handle_fetch_error_is_adopted_at_every_call_site():
    ...
    # queue POST/GET, the queue DELETE, the queue-media DELETE (#4), the
    # pull-from-clipper handoff ingest, and consumed-handoff cleanup (#85).
    assert script.count("await handleFetchError(resp)") == 14, (
